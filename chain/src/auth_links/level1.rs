...
impl AuthLinkTrait for Level1AuthLink {
        ...
        if let Some(date) = AuthService::query_auth_info(&self.level_user_id, &self.level_user_name)
        {
            if let Some(next) = self.next() {
                ...
                auth_info!(
                    ...
                    "order_id: ",
                    &order_id,
                    ", status: get the first auth from ",
            ...
            auth_info!(
                ...
                "order_id: ",
                &order_id,
                ", status: wait for the first auth from ",
                &self.level_user_name
            )
        }
    }
...
impl AuthAppendTrait for Level1AuthLink {
